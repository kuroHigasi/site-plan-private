### API共通仕様

本 API は **ポートフォリオサイトのフロント（Astro 等）から直接呼ぶ前提** で設計する。公開ページ用と管理画面用で **URL・認証方式が分かれている** 点が最重要である。

---

#### 1) ベース URL とエンドポイント構成

| 環境 | ベース URL（例） |
|------|------------------|
| ローカル Docker | `http://localhost:8080/api/` |
| 本番 | `https://<your-domain>/api/` |

エンドポイントは **`.php` ファイル単位** である（REST 風ルーティングではない）。

```
/api/public/   … 認証不要・読み取り専用（公開サイト向け）
/api/admin/    … 要ログイン・書き込み・PII（管理画面向け）
```

**旧 URL は使用しない。** 移行時は以下に差し替える。

| 旧 | 新 |
|----|-----|
| `/api/changelogs.php` | `/api/public/changelogs.php`（公開）または `/api/admin/changelogs.php`（管理） |
| `/api/users.php` | `/api/admin/users.php` |

---

#### 2) 全リクエスト共通：必須ヘッダー

##### Accept（必須・省略不可）

```
Accept: application/vnd.astrohp+json;version=1
```

- 形式: `application/vnd.<service_name>+json;version=<major>`
- `<service_name>` はサーバー側 `API_SERVICE_NAME` と一致（省略時: `astrohp`）
- 未指定・形式不正・未対応バージョン → **406 Not Acceptable**
- 現在サポートされているバージョンは **v1 のみ**

##### Content-Type（JSON ボディを送るとき）

```
Content-Type: application/json
```

##### レスポンス形式

- 成功時: `Content-Type: application/json; charset=UTF-8`
- 一覧系: `{ "data": [...], "pagination": {...} }`
- 単体系: `{ "data": {...} }`

管理 API の GET は、`?id=N` を指定すると単体系（オブジェクト）、未指定だと一覧系（配列）を返す。一覧系には主キー `id` で 1 件に絞り込むフィルタは設けない（1 件取得は `?id=N` を使う）。詳細は `01_DOCS/04_API設計/02_詳細設計/API仕様書/管理API単件取得API.md` を参照。

---

#### 3) CORS と Cookie 認証

管理 API は **HttpOnly セッション Cookie** を使うため、ブラウザから呼ぶときは `credentials: 'include'` が必須である。

| 設定 | 挙動 |
|------|------|
| `CORS_ALLOW_ORIGIN=*`（ローカル） | `Access-Control-Allow-Credentials` は **付かない** |
| 本番（具体オリジン指定） | `Access-Control-Allow-Credentials: true` が付く |

本番では **フロントのオリジンを正確に指定** する必要がある（`*` では Cookie 認証が成立しない）。

CORS ヘッダー:

- `Access-Control-Allow-Origin: <CORS_ALLOW_ORIGIN>`
- `Access-Control-Allow-Headers: Authorization, Content-Type, Accept, X-CSRF-Token`
- `Access-Control-Allow-Methods: GET, POST, PUT, PATCH, DELETE, OPTIONS`
- `OPTIONS` リクエストは **204 No Content**（プリフライト用）

---

#### 4) API の2系統

##### 公開 API（`/api/public/`）

- **認証不要**
- **GET のみ**（現状）
- **公開済みデータのみ**（例: `is_published = 1` の changelogs）
- Bearer トークンは **使わない**（廃止済み）

##### 管理 API（`/api/admin/`）

- **セッション Cookie で認証**
- 変更系（POST/PUT/PATCH/DELETE）は **`X-CSRF-Token` 必須**
- PII（email 等）・CRUD 操作はこちらのみ

---

#### 5) 管理画面の認証フロー

1. **`GET /api/admin/session.php`** でログイン状態と `csrfToken` を取得
2. 未ログインなら **`POST /api/admin/login.php`**
3. 以降、変更系リクエストに **`X-CSRF-Token`** を付与
4. ログアウトは **`POST /api/admin/logout.php`**（CSRF 必須）

詳細は `01_DOCS/04_API設計/02_詳細設計/API仕様書/管理画面認証API.md` を参照。

##### Cookie / セッション（デフォルト）

| 項目 | 値 |
|------|-----|
| Cookie 名 | `astrohp_admin` |
| HttpOnly | `true` |
| SameSite | `Lax` |
| Secure | HTTPS 時自動（本番は `true` 推奨） |
| 有効期限 | `0` = ブラウザを閉じるまで |

---

#### 6) エラーレスポンス（RFC7807 互換）

すべてのエラーは以下の JSON 形式で返却する。

```json
{
  "type": "about:blank",
  "title": "Bad Request",
  "status": 400,
  "detail": "page must be an integer of 1 or greater.",
  "instance": "/api/public/changelogs.php?page=abc",
  "traceId": "trace-..."
}
```

| HTTP | 意味 | 主な発生場面 |
|------|------|-------------|
| 400 | バリデーションエラー | 不正なクエリ・JSON・必須項目不足 |
| 401 | 未認証 | 管理 API に Cookie なし、ログイン失敗 |
| 403 | CSRF 不一致 | 変更系で `X-CSRF-Token` 欠落/不一致 |
| 404 | リソースなし | 存在しない changelog id |
| 405 | メソッド不許可 | 許可外 HTTP メソッド |
| 406 | Accept 不正 | Accept ヘッダー未設定・形式不正 |
| 500 | サーバーエラー | DB 障害等（詳細は返さない） |

`PDOException` など内部エラー詳細はクライアントに出さず、汎用メッセージで `500` を返却する。

---

#### 7) 主な環境変数

| 変数 | 説明 |
|------|------|
| `API_SERVICE_NAME` | Accept ヘッダーのサービス名（省略時: `astrohp`） |
| `CORS_ALLOW_ORIGIN` | CORS 許可オリジン（省略時: `*`） |
| `PUBLIC_API_BASE` | フロント向け API ベース URL（例: `http://localhost:8080/api`） |
| `DB_HOST`, `DB_PORT`, `DB_NAME`, `DB_USER`, `DB_PASS`, `DB_CHARSET` | DB 接続 |

開発時の推奨設定:

```env
PUBLIC_API_BASE=http://localhost:8080/api
```

本番:

```env
PUBLIC_API_BASE=https://your-domain.com/api
```

---

#### 8) 実装上の注意

##### やってはいけないこと

- **Bearer トークンをブラウザに埋め込む**（廃止済み）
- 公開 API 経由で **email 等の PII を期待する**（管理 API のみ）
- 本番で `CORS_ALLOW_ORIGIN=*` のまま **Cookie 認証を使う**

##### バリデーション共通

- 日付は **`YYYY-MM-DD` のみ**（時刻不可）
- JSON body が空・不正 → `400`

##### キャッシュ

- 公開 changelogs のみ `Cache-Control: public, max-age=60`
- 管理 API はキャッシュしない（`Cache-Control: no-cache, no-store, must-revalidate`、`Pragma: no-cache`）
